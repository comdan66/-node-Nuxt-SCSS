...
const Factory = function(type, file) {
  ...
  if (isSub(Factory.config.icon, file) && ext == '.css' && last == 'style.css')
    return Queue.push(Factory.Icon(type, file), 357)

  if (isSub(Factory.config.scss, file) && ext == '.scss')
...
Factory.Icon = function(type, file) {
  ...
  this.scss  = Factory.config.scss + this.face + '.scss'
}
...
Factory.Icon.prototype.parse = function(data) {
  const importStr = '@import "Lalilo";', basePath = '../icon/', contents = ['//', '// @author      OA Wu <oawu.tw@gmail.com>', '// @copyright   Copyright (c) 2015 - ' + new Date().getFullYear() + ', Lalilo', '// @license     http://opensource.org/licenses/MIT  MIT License', '// @link        https://www.ioa.tw/', '//', '', importStr]
...
Factory.Scss = function(type, file) {
  return this instanceof Factory.Scss
    ? Factory.call(this, type, file, this.dirs = Path.relative(Factory.config.scss, file).split(Path.sep).filter(t => t.length), this.css = Factory.config.css + [...this.dirs.slice(0, -1), Path.basename(this.dirs.pop(), '.scss') + '.css'].join(Path.sep))
...
Factory.Scss.prototype.build = function(done) {
  if (this.type == 'first')
    return SCSS.file(this.file, (error, result) => error
      ...
      : verifyDirs(Factory.config.css, this.dirs) !== true
